...
define default-build-package
	...
	--cmake-args ${COMMON_CMAKE_ARGS} $(EXTRA_CMAKE_ARGS) -DCMAKE_POSITION_INDEPENDENT_CODE=ON && \
...
launch-g1-dummy-sim-cm:
	cd ${build_dir} && \
	source ${ros_source_file} && \
	source install/setup.bash && \
	export WB_HUMANOID_MPC_SRC=/wb_humanoid_mpc_ws/src/wb_humanoid_mpc && \
	ros2 launch g1_centroidal_mpc dummy_sim_cm.launch.py
