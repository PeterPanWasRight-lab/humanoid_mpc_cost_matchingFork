...
/**
 * RK4 integrator on fixed MPC grid.
 ...
 *  ODE:
 *    xdot = f_theta(x,u)
 *    Sdot = A_theta(x,u) * S + B(x,u)
 *  where:
 *    A_theta = df_theta/dx = row-scaled version of A_nom
 *    B = df_theta/dtheta_dyn is sparse on the 6 momentum-dot rows.
 *
 * We integrate (x,S) with classic RK4 using the same (t,u_of_t) sampling at each stage.
 */
class FixedGridRK4Rollout {
 public:
  FixedGridRK4Rollout(ocs2::SystemDynamicsBase& dyn, size_t idxHlinDot, size_t idxHangDot);
